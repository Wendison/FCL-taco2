...
class LoadInputsAndTargets(object):
    ...
    def _create_batch_tts(self, x_feats_dict, y_feats_dict, uttid_list, eos):
        ...
        if self.load_input:
            ...
            if self.use_second_target:
                ...
                # print('spcs:', spcs, len(spcs))
                # third is f0, 4th is energy
                if len(list(x_feats_dict.values())) > 2:
                    f0 = list(x_feats_dict.values())[2]
                    f0 = [f0[i] for i in nonzero_sorted_idx]
                    f0_name = list(x_feats_dict.keys())[2]
                    # print('f0:', f0, len(f0))
                    ene = list(x_feats_dict.values())[3]
                    ene = [ene[i] for i in nonzero_sorted_idx]
                    ene_name = list(x_feats_dict.keys())[3]
            ...
            if len(list(x_feats_dict.values())) == 4 and self.use_second_target:
                return_batch = OrderedDict(
                    [(x_name, xs), (y_name, ys), (spembs_name, spembs), (spcs_name, spcs), (f0_name, f0), (ene_name, ene)]
                )
            else:
                return_batch = OrderedDict(
    ...
    def _get_from_loader(self, filepath, filetype):
        ...
        if filetype == "hdf5":
            ...
            #                "filetype": "npy"},
            if not self.keep_all_data_on_mem:
                return torch.load(filepath).cpu().numpy()
            if filepath not in self._loaders:
                self._loaders[filepath] = torch.load(filepath).cpu().numpy()
            return self._loaders[filepath]
