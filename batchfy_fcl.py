...
def batchfy_by_bin(
    ...
    while True:
        ...
        next_size = 0
        max_olen = 0
        while next_size < batch_bins and (start + b) < length: # visit each sample, find the sample with max bins
            # max bins is max_olen, and the max-possible bins sum is next_size=max_olen+ilen)*(b+1), 
            # b is num of visisted samples, so we can find the next largest num (b+1) of samples that can be fit
            # into batch_bins
            ilen = int(sorted_data[start + b][1][ikey][0]["shape"][0]) * idim
            olen = int(sorted_data[start + b][1][okey][0]["shape"][0]) * odim # (start+b)th mel-spec bins
            if olen > max_olen:
                max_olen = olen
...
def make_batchset(
    ...
    swap_io=False,
    mt=False,
    count="auto", # for transformer-TTS experiments, count will be automatically set to 'bin'
    batch_bins=0,
    batch_frames_in=0,
    batch_frames_out=0,
    ...
    print(f'\n\n before swap, batch_sort_key: {batch_sort_key} \n\n')
    batch_sort_axis = 0
    if swap_io:
        # for TTS
        ikey = "output"
    ...
    print(f'\n\n after swap, batch_sort_key: {batch_sort_key} \n\n')
    ...
    for d in category2data.values():
        ...
        batches_list.append(batches) # each element in batches is a list (dlist) which contains info for each samples
                                    # len(dlist) = # of samples, i.e., batch_size for one iteration, 
                                    # so batch_size is dynamically changed during the training,
                                    # and dlist is fixed during the whole training
